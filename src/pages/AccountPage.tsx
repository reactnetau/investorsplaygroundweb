    ...
    try {
      if (profile) {
        const [portfolios] = await Promise.all([
          client.models.Portfolio.list(),
        ]);
        const allHoldings = await Promise.all(
          (portfolios.data ?? []).map((p) =>
            client.models.Holding.listByPortfolio(p.id)
          )
        );
        await Promise.all([
        ...
        {pro ? (
          <div className="space-y-3">
            ...
            <div className="p-3 bg-brand-50 border border-brand-100 rounded-xl text-sm text-brand-800">
              Your subscription is managed through the Investors Playground mobile app. To cancel, open the app and manage your subscription there.
            </div>
          </div>
        ) : (
          <div className="space-y-3">
            ...
            <div className="p-4 bg-brand-50 border border-brand-100 rounded-xl">
              ...
              <p className="text-sm text-gray-600 mb-1">
                Pro subscriptions are managed through the Investors Playground mobile app.
              </p>
              <p className="text-xs text-gray-400">Download on iOS or Android and subscribe from within the app.</p>
            </div>
          </div>
        )}
      </div>
